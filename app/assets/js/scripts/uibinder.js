...
async function showMainUI(data){
    ...
    try {
        ...
        console.error('Error during prepareSettings:', err)
    }
    ...
    try {
        ...
        console.error('Error during server selection setup:', err)
    }

    setTimeout(() => {
        document.getElementById('frameBar').style.backgroundColor = 'rgba(0, 0, 0, 0.5)'
        document.body.style.backgroundImage = 'url(assets/images/relaxzone_banner.jpg)'
        $('#main').show()
...
async function handleDistributionResult(res) {
    if(distroLoaded) return
    distroLoaded = true
    if(res) {
        try {
            const data = await DistroAPI.getDistribution()
            ...
            console.error('Failed to load distribution data in uibinder:', err)
...
ipcRenderer.on('distributionIndexDone', async (event, res) => {
    await handleDistributionResult(res)
})

// Request cached distribution index in case preloader completed before renderer was ready
ipcRenderer.send('requestDistributionIndexDone')

// Fallback: If no event arrives within 200ms, load distribution directly
setTimeout(async () => {
    if(!distroLoaded) {
        try {
            const data = await DistroAPI.getDistribution()
            if(data != null && !distroLoaded) {
                await handleDistributionResult(true)
            }
        } catch(e) {
            console.warn('Fallback distro load attempt:', e)
        }
    }
}, 200)
