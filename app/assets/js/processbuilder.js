...
class ProcessBuilder {
                ...
                if(argDiscovery.test(args[i])){
                    ...
                    switch(identifier){
                        ...
                        case 'natives_directory':
                            ...
                            break
                        case 'launcher_name':
                            val = args[i].replace(argDiscovery, 'RelaxZone-Launcher')
                            break
    ...
    /**
     * Ensure the target multiplayer server is configured in the in-game servers.dat list.
     */
    ensureServersDat(){
        try {
            const serversPath = path.join(this.gameDir, 'servers.dat')
            const targetAddress = this.server.rawServer.address || `${this.server.hostname}:${this.server.port}`
            const targetName = '§6§lRelax Zone §r§b悠然之境'

            let existingServers = []
            if(fs.existsSync(serversPath)){
                try {
                    const buf = fs.readFileSync(serversPath)
                    const parsed = ProcessBuilder.readNBT(buf)
                    if(parsed && Array.isArray(parsed.servers)){
                        existingServers = parsed.servers
                    }
                } catch(e) {
                    logger.warn('Failed to parse existing servers.dat, will create fresh:', e.message)
                }
            }

            const cleanTarget = targetAddress.toLowerCase().replace(/:25565$/, '')
            const foundIndex = existingServers.findIndex(s => {
                const cleanS = (s.ip || '').toLowerCase().replace(/:25565$/, '')
                return cleanS === cleanTarget
            })

            if(foundIndex >= 0){
                if(existingServers[foundIndex].name === 'Minecraft 伺服器' || !existingServers[foundIndex].name){
                    existingServers[foundIndex].name = targetName
                    const datBuf = ProcessBuilder.writeServersDat(existingServers)
                    fs.writeFileSync(serversPath, datBuf)
                }
            } else {
                existingServers.unshift({
                    name: targetName,
                    ip: targetAddress,
                    acceptTextures: 1
                })
                const datBuf = ProcessBuilder.writeServersDat(existingServers)
                fs.writeFileSync(serversPath, datBuf)
                logger.info(`Pre-populated ${targetName} (${targetAddress}) into servers.dat`)
            }
        } catch(err) {
            logger.error('Error ensuring servers.dat:', err)
        }
    }

    static readNBT(buffer) {
        let offset = 0
        function readByte() { return buffer.readInt8(offset++) }
        ...
        function readString() { const len = readShort(); const s = buffer.toString('utf8', offset, offset + len); offset += len; return s }
        function parseTag(type) {
            if (type === 0) return null
            if (type === 1) return readByte()
            if (type === 2) return readShort()
            if (type === 3) return readInt()
            if (type === 4) { offset += 8; return 'long' }
            if (type === 5) { offset += 4; return 'float' }
            if (type === 6) { offset += 8; return 'double' }
            if (type === 7) { const len = readInt(); offset += len; return 'byte[]' }
            if (type === 8) return readString()
            if (type === 9) {
                const subType = readByte()
                const len = readInt()
                const arr = []
                for (let i = 0; i < len; i++) arr.push(parseTag(subType))
                return arr
            }
            if (type === 10) {
                const obj = {}
        ...
        readString()
        return parseTag(rootType)
    }

    static writeServersDat(servers) {
        const bufs = []
        function writeByte(v) { const b = Buffer.alloc(1); b.writeInt8(v); bufs.push(b) }
        function writeShort(v) { const b = Buffer.alloc(2); b.writeInt16BE(v); bufs.push(b) }
        function writeInt(v) { const b = Buffer.alloc(4); b.writeInt32BE(v); bufs.push(b) }
