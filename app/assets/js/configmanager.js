...
const dataPath = path.join(sysRoot, '.relaxzone-launcher')
...
/**
 * Move data from the previous launcher directory on the first run after the
 * Relax Zone branding migration. This keeps existing game files and Java
 * installations available without exposing the legacy path in the UI.
 */
function migrateLegacyDataDirectory(){
    const configuredDataPath = config?.settings?.launcher?.dataDirectory
    if(typeof configuredDataPath !== 'string' || path.resolve(configuredDataPath) !== path.resolve(legacyDataPath)){
        return
    }
    ...
    if(fs.existsSync(legacyDataPath) && !fs.existsSync(dataPath)){
        try {
            ...
            logger.info('Migrated launcher data to the Relax Zone directory.')
        } catch(err) {
            logger.warn('Unable to migrate the previous launcher data directory:', err)
            return
        }
    }
